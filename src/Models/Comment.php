...
class Comment{
    ...
    public static function getCommentsByPostId(){
        $TABLE= self::$TABLE;
        $TABLE2= self::$TABLE2;
        $result = DB::query("SELECT $TABLE.* FROM $TABLE INNER JOIN $TABLE2 ON $TABLE2.id=$TABLE.post_id");
    ...
    public static function modify($id, $author, $content){
        $sql = DB::prepare("UPDATE ".self::$TABLE." SET author=:author ,content=:content  WHERE id=:id ;");
